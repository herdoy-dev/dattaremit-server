...
class ZynkService {
  async createEntity(userId: string) {
    return Sentry.startSpan(
      { name: "zynk.createEntity", op: "http.client", attributes: { "zynk.endpoint": "entity/create" } },
      async () => {
  ...
  async startKyc(userId: string) {
    return Sentry.startSpan(
      { name: "zynk.startKyc", op: "http.client", attributes: { "zynk.endpoint": "kyc/start" } },
      async () => {
  ...
  async getKycStatus(userId: string) {
    return Sentry.startSpan(
      { name: "zynk.getKycStatus", op: "http.client", attributes: { "zynk.endpoint": "kyc/status" } },
      async () => {
        const user = await requireUserWithEntity(userId);
  ...
  async addExternalAccount(userId: string, data: ZynkAddExternalAccountData) {
    return Sentry.startSpan(
      { name: "zynk.addExternalAccount", op: "http.client", attributes: { "zynk.endpoint": "account/external" } },
      async () => {
        // Phase 1: Read and validate data outside transaction
  ...
  async addDepositAccount(userId: string, data: ZynkAddDepositAccountData) {
    return Sentry.startSpan(
      { name: "zynk.addDepositAccount", op: "http.client", attributes: { "zynk.endpoint": "account/deposit" } },
      async () => {
        // Phase 1: Read and validate data outside transaction
        const user = await prismaClient.user.findUnique({ where: { id: userId } });
        if (!user) {
  ...
  async generatePlaidLinkToken(
    ...
    return Sentry.startSpan(
      { name: "zynk.generatePlaidLinkToken", op: "http.client", attributes: { "zynk.endpoint": "plaid/link-token" } },
      async () => {
        const user = await requireUserWithEntity(userId);
